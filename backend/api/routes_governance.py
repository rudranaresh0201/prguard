...
async def trigger_index(_: None = Depends(verify_internal_token)):
    """Index this repo's own source into CodeRAG's collection, offloaded to a
    real thread (not just a deferred BackgroundTask -- that still runs on the
    single event-loop thread and blocks it for CPU-bound work) so the single
    worker process can keep serving other requests while this runs. Never
    wired up automatically anywhere -- must be called explicitly at least
    once. Poll /governance/health for codebase_chunks to know when it's done."""
    import asyncio
    asyncio.create_task(asyncio.to_thread(_run_index))
